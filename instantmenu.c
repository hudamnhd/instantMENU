...
// instantASSIST: items are a single letter with a description appearing on
// selection.
...
static const int outputoffset[ItemLast] = {
    [ItemNormal] = 0,  [ItemComment] = 1, [ItemColoredComment] = 4,
    [ItemColored] = 2, [ItemIcon] = 6,
};
...
static int drawitem(struct item *item, int x, int y, int w) {
    ...
    if (itemcategory == ItemColored) {
        if (item->text[2] == ' ') {
            ...
            dest[6] = '\0';
            drw_text(drw, x, y, temppadding, lineheight, temppadding / 2.6,
                     dest + 3, 0, item == sel);
    ...
    if (item == sel)
        sely = y;
    return drw_text(
        drw, x + ((itemcategory == ItemIcon) ? temppadding : 0), y,
        commented ? bh : (w - ((itemcategory == ItemIcon) ? temppadding : 0)),
        bh,
        commented ? (bh - drw_fontset_getwidth(drw, (output))) / 2 : lrpad / 2,
        output + outputoffset[itemcategory], 0,
        (itemcategory == ItemColoredComment || item == sel));
}
...
static void drawmenu(void) {
    ...
    if (lines > 0) {
        ...
        if (next) {
            ...
            drw_setscheme(drw, scheme[SchemeNorm]);
            if (tempnumer)
                drw_text(drw, mw - w - TEXTW(numbers), 0, w, bh, lrpad / 2, ">",
                         0, 0);
        }
    }
    ...
    drw_setscheme(drw, scheme[SchemeNorm]);
    if (tempnumer) {
        drw_text(drw, mw - TEXTW(numbers) - (rightcmd ? arrowwidth : 0), 0,
                 TEXTW(numbers), bh, lrpad / 2, numbers, 0, 0);
    }
    if (lines > 0) {
        if (leftcmd) {
            drw_setscheme(drw, scheme[SchemeHighlight]);
...
void animatesel() {
    ...
    while (time < framecount) {
        ...
        drw_rect(
            drw, 0,
            sely + 4 - (easeOutQuint(((double)time / framecount)) * (sely + 4)),
...
void animaterect(int x1, int y1, int w1, int h1, int x2, int y2, int w2,
    ...
    while (time < framecount) {
        ...
        drw_rect(drw, x1 + (x2 - x1) * timefactor, y1 + (y2 - y1) * timefactor,
                 w1 + (w2 - w1) * timefactor, h1 + (h2 - h1) * timefactor, 1, 1,
                 0);
        drw_map(drw, win, 0, 0, mw, mh);
        time++;
        usleep(19000);
    ...
    if (lines > 0) {
        ...
        if (columns > 0) {
            ...
            for (item = curr; item != next;) {
                ...
                // event in y range
                if (ev->y >= checky && ev->y <= (checky + h) &&
                    ev->x >= checkx && ev->x <= (checkx + colwidth)) {
                    if (sel == item)
                        return;
                    sel = item;
                    if (sel) {
...
static void setup(void) {
    ...
    if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
        ...
        if (centered) {
            ...
            x = rightxoffset ? info[i].x_org + info[i].width - dmx - mw -
                                   2 * border_width
                             : info[i].x_org + dmx;
...
static void usage(void) {
    die("usage: instantmenu [-bfirnPv] [-l lines] [-g columns] [-p prompt] [-m "
        "monitor]\n"
        "             [-x xoffset] [-xr right xoffset] [-y yoffset] [-w "
        "width]\n"
        "             [-h height] [-fn font]\n"
...
int main(int argc, char *argv[]) {
    ...
    for (i = 1; i < argc; i++)
        ...
        if (!strcmp(argv[i], "-v")) { /* prints version information */
            ...
            topbar = 0;
                         ...
                         "-r")) /* reject input if it results in no matches */
            rejectnomatch = 1;
        else if (!strcmp(argv[i],
            ...
            followcursor = 1;
        else if (!strcmp(argv[i], "-S")) /* confirm using the space key TODO
            ...
            exact = 1;
            fuzzy = 0;
        } else if (!strcmp(argv[i], "-H")) { /* makes instantmenu take the full
                                                screen height */
            fullheight = 1;
        } else if (!strcmp(argv[i],
            ...
            rightcmd = argv[++i];
                         ...
                         "-lc")) /* adds prompt to left of input field */
            leftcmd = argv[++i];
        /* these options take one argument */
            ...
            prompt = argv[++i];
                         ...
                         "-q")) /* adds prompt inside of the input field */
            searchtext = argv[++i];
            ...
            if (!fullheight) {
                ...
                lineheight =
                    MAX(lineheight, 8); /* reasonable default in case of value
                                           too small/negative */
            }
    ...
    if (dmw <= -1) {
        int maxw =
            max_textw() * 1.3 * MAX(columns, 1) + (prompt ? TEXTW(prompt) : 0);
        if (dmw * (-1) > maxw) {
            dmw = dmw * (-1);
        } else {
            dmw = maxw;
